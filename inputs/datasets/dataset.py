...
class Dataset():
    """Constructs dataset for multiple date file
    """
    def __init__(self, name, instance_dict=dict()):
        """Initializes a dataset: dataset name, this dataset contains multiple text readers for multiple data files
    ...
    def add_instance(self, name, instance, reader, is_count=False, is_train=False):
        """Adds a instance to the dataset

        Arguments:
            name {str} -- instance name
            instance {Instance} -- instance
            reader {TextReader} -- text reader correspond to the instance

        Keyword Arguments:
            is_count {bool} -- whether the instance participates in counting (default: {False})
            is_train {bool} -- whether the instance is training data (default: {False})
        """

        self.instance_dict[name] = {'instance': instance, 'reader': reader, 'is_count': is_count, 'is_train': is_train}
    ...
    def build_dataset(self,
        ...
        if counter is not None:
            for instance_name, instance_settting in self.instance_dict.items():
                if instance_settting['is_count']:
                    instance_settting['instance'].count_vocab_items(counter, instance_settting['reader'])

            # construct vocabulary from counter
            vocab.extend_from_counter(counter, min_count, no_pad_namespace, no_unk_namespace, contain_pad_namespace,
                                      contain_unk_namespace)

        # add extra tokens, this operation should be executed before adding pretrained_vocab
        ...
        if pretrained_vocab is not None:
            vocab.extend_from_pretrained_vocab(pretrained_vocab, intersection_namespace, no_pad_namespace,
                                               no_unk_namespace, contain_pad_namespace, contain_unk_namespace)
        ...
        for instance_name, instance_settting in self.instance_dict.items():
            ...
            self.instance_dict[instance_name]['vocab_dict'] = instance_settting['instance'].get_vocab_dict()

            logger.info("{} dataset size: {}.".format(instance_name, self.instance_dict[instance_name]['size']))
            for key, seq_len in instance_settting['reader'].get_seq_lens().items():
                logger.info("{} dataset's {}: max_len={}, min_len={}.".format(instance_name, key, max(seq_len),
                                                                              min(seq_len)))

    def get_batch(self, instance_name, batch_size, sort_namespace=None):
        """Gets batch data and padding
            ...
            for namespace in dataset:
                ...
                if namespace in self.wo_padding_namespace:
                    ...
                    if isinstance(dataset[namespace][0][0], list):
                        max_char_len = 0
                        for id in sorted_ids:
                            max_char_len = max(max_char_len, max(len(item) for item in dataset[namespace][id]))
                        for id in sorted_ids:
                            padding_sent = []
                            mask = []
                            for item in dataset[namespace][id]:
                                padding_sent.append(item + [padding_idx] * (max_char_len - len(item)))
                            ...
                            mask = mask + [[0] * max_char_len] * (max_namespace_len - len(dataset[namespace][id]))
                            batch[namespace].append(padding_sent)
                            batch[namespace + '_mask'].append(mask)
                    else:
                        for id in sorted_ids:
                            batch[namespace].append(dataset[namespace][id] + [padding_idx] *
                                                    (max_namespace_len - len(dataset[namespace][id])))
                            batch[namespace + '_mask'].append([1] * len(dataset[namespace][id]) + [0] *
                                                              (max_namespace_len - len(dataset[namespace][id])))
    ...
    def set_wo_padding_namespace(self, wo_padding_namespace):
        """Sets without paddding namespace
