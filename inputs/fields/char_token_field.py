...
class CharTokenField(Field):
    """A field for char sequence type
    """
    ...
    def count_vocab_items(self, counter, sentences):
        """Counts token's char in sentences, then updates counter
    ...
    def index(self, instance, vocab, sentences):
        ...
        for sentence in sentences:
            token_num_repr = []
            for token in sentence[self.source_key]:
                token_num_repr.append([vocab.get_token_index(char, self.vocab_namespace) for char in token])
