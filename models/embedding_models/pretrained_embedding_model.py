...
class PretrainedEmbedModel(nn.Module):
    ...
    def forward(self, batch_inputs):
        ...
        if 'wordpiece_segment_ids' in batch_inputs:
            batch_seq_pretrained_encoder_repr, batch_cls_repr, batch_pretrained_hidden_states, batch_bert_attentions = self.pretrained_encoder(
                batch_inputs['wordpiece_tokens'], batch_inputs['wordpiece_segment_ids'])
            batch_inputs['batch_bert_attentions'] = batch_bert_attentions
        else:
            batch_seq_pretrained_encoder_repr, batch_cls_repr, batch_pretrained_hidden_states, batch_bert_attentions = self.pretrained_encoder(
